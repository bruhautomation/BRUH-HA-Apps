...
Point BRigt at your music. It analyzes each track once — tempo, the beat
...
BRigt works with no LIFX bulbs at all (the aux-light channel still runs), and
...
a Google/Nest speaker, read that section *before* concluding BRigt is broken.
...
1. Add this repository to **Settings → Add-ons → Add-on store → ⋮ →
   Repositories**.
2. Install **BRigt** and start it.
3. Accept the discovered **BRigt** integration under **Settings → Devices &
...
2. **Play clicks & listen** — the real measurement. Open the panel *on your
   phone*, in the room with the speaker. BRigt plays a 13-second click track;
...
Chromecast has no route to BRigt's panel and would not be allowed through it
if it did. BRigt creates that folder at startup.
...
at it (and keep a `local:` entry for `/media`, which is what BRigt builds its
links from).
...
- `auto` — Claude designs the show when **brAIn** is installed (BRigt
...
*Default: on.* Deploys the companion `brigt` integration and runs the bridge
that carries service calls: `brigt.party_mode`, `brigt.start_show`,
`brigt.stop_show`, and the show-status sensor. Turn it off and the panel
...
This is the most common problem, it is almost never BRigt, and there is one
...
BRigt builds `media-source://media_source/local/<path>` from where a file
...
`/media` belongs to root and the panel runs as a non-root user, so BRigt
creates `/media/brigt` for itself at startup. If that failed, the message
...
systems drop broadcast traffic between bands or VLANs. BRigt sets
...
Supervisor now assigns BRigt's panel port. If you are on an older version,
update.
...
## Where BRigt keeps things

| Path | What |
|---|---|
| `/media/<your folders>` | Your music. BRigt only ever reads it. |
| `/media/brigt/calibration.wav` | The click track, regenerated when missing |
| `/config/.brigt/` | The shared folder the integration talks through, plus the mirrored show state |
| `/config/custom_components/brigt/` | The companion integration, deployed at startup |
...
Nothing BRigt does is destructive to your Home Assistant configuration, and
