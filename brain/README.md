...
| | |
| --- | --- |
| **Runs Home Assistant** | 82 native tools for reading and controlling (ESPHome devices, Music Assistant, BRUH Minecraft, BRight and BRUH Print included), 65 registry-management services for the parts that normally live behind the Settings UI, and a real shell in `/config` for everything that's still YAML. Areas, floors, labels, devices, entities, integrations, helpers, zones, people, users, dashboards, blueprints, statistics — create, rename, move, disable, delete. |
