...
| | |
| --- | --- |
| **Runs Home Assistant** | 64 native tools for reading and controlling (ESPHome devices included), 65 registry-management services for the parts that normally live behind the Settings UI, and a real shell in `/config` for everything that's still YAML. Areas, floors, labels, devices, entities, integrations, helpers, zones, people, users, dashboards, blueprints, statistics — create, rename, move, disable, delete. |
| **Finds what's broken** | A dead battery, a sensor that quietly stopped reporting, a device stuck unavailable, an automation that can never fire. Press **Fix it** and it shows you what it would change first, makes the change on **Apply**, and can put the files back with **Undo**; press **Wrong** and say why, and it never makes that mistake again in new words. Every one waiting on you is also on Home Assistant's own Repairs page, with the same buttons. |
