...
#   /config/.brain/requests/   - incoming conversation requests (JSON)
#   /config/.brain/responses/  - outgoing conversation responses (JSON)
#   /config/.brain/sessions/   - conversation_id -> Claude session uuid
#
...
CLAUDE_TIMEOUT="${BRAIN_ASSIST_TIMEOUT:-105}"
...
If unsure of an entity_id, call get_all_states with a domain filter first."
    ...
    if [ "${BRAIN_MEMORY_INJECTION:-true}" != "false" ]; then
        local memory_dir="${BRAIN_MEMORY_DIR:-/config/.brain/memory}"
        local memory_text=""
