...
# shared auth file that other BRUH add-ons (like BRUH Insights) read:
#
#   /config/.bruh_claude/secrets/claude_auth.json
...
AUTH_DIR="${BRUH_AUTH_DIR:-/config/.bruh_claude/secrets}"
AUTH_FILE="$AUTH_DIR/claude_auth.json"

# Must match OAUTH_TOKEN_RE in bruh-insights/panel/claude_client.py
...
(0600, owned by the claude user). Other BRUH add-ons (like BRUH Insights)
...
write_auth_file() {
    ...
    echo -e "${GREEN}Other BRUH add-ons (like BRUH Insights) will now use this login automatically.${NC}"
}
