...
- **Power Tools** — 41 registry-management services (`bruh_claude.*`): create/rename areas, floors, labels and zones, rename entities and devices, enable/disable/hide entities, reload integrations, import blueprints, backfill statistics, find orphaned references, flag repair issues — all validated and admin-gated, no `.storage` hand-edits (adapted from [Spook](https://github.com/frenck/spook), MIT)
