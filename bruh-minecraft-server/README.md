...
- **Independent worlds.** Every world has its own settings — one can be creative, another survival. Gameplay (gamemode, difficulty, world-gen, whitelist, …) is edited per-world in the panel's **Server Properties** tab; switching worlds loads that world's own config.
- **Offline-mode / no-Xbox play done right.** Set a world's `online-mode: false` and the add-on silently fixes every downstream setting (`enforce-secure-profile`, Geyser `auth-type`, Floodgate removal, `validate-bedrock-login`) so LAN / family / kids-without-Xbox setups just work.
