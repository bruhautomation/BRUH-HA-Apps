...
box may already own. BRight shipped 8095 — picked only because BRUH Minecraft
...
Order: `BRIGHT_PANEL_PORT` first (run.sh resolved it once and exported it into
the process tree *and* into /data/.bright_env, the only route an answer has
...
log = logging.getLogger("bright.panel.port")
...
def bind(host: str, port: int, *, attempts: int = 5,
    ...
    raise PortInUse(
        errno.EADDRINUSE,
        f"Something else on this machine already owns port {port}. BRight runs "
        f"with host_network, so that is a port on the HOST, not one inside "
        f"the container. Free it and restart BRight.",
    ) from last
