...
can go wrong afterwards goes wrong somewhere BRight cannot see: Core resolves
...
def flat(value: object, limit: int = 200) -> str:
    ...
    Every detail below quotes something BRight did not write — an entity id,
...
def file_step(path: Path | None, expected_size: int | None = None) -> dict:
    ...
    try:
        ...
        return _step("file", False, f"{path} is not readable: {flat(exc)}",
                     "BRight can only play files it can see under /media.")
